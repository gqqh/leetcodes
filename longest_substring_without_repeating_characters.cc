...
// 求string的最长不含重复字母的子串，使用一个int hash[256]记录每一个字母上一次
// 出现的位置，同时使用flag记录上一次出现重复字母的位置，注意，此处记录的是
// 重复字母的第一个字母。然后每次比较新字母的hash值和flag的位置关系判断新字母
// 是否应该加入：如果hash值 <= flag则可以加入，此时应当比较i-flag与max的大小关系，
// 如果已经比max大了，表示新的最长串出现了。否则表示重复，则更新flag值。
// 注意每一新字母都要更新其hash值。
@param  s 输入串
@return   最大无重复字母子串长度

int lengthOfLongestSubstring(char *s) {
    int flag = -1; //上一次出现重复字母的位置
    int max = 0;
